<template>
    <div class="col-wrapper" :style="getWrapperHeight">
        <div class="wheel-list" :style="getListTop" ref="wheel">
            <div
                class="wheel-item"
        ...
        <div class="wheel-cover" :style="getCoverStyle"></div>
        <div class="wheel-divider" :style="getDividerStyle"></div>
    </div>
</template>
<script>
class Animate {
...
const a = -0.003; // 加速度
const radius = 150; // 半径
const lineHeight = 56; // 文字行高
let isInertial = false; // 是否正在惯性滑动
// 根据三角形余弦公式
// 反余弦得到弧度再转换为度数,这个度数是单行文字所占有的。
let deg = Math.round(
    (Math.acos(
        (radius * radius + radius * radius - lineHeight * lineHeight) /
            (2 * radius * radius)
    ) *
        180) /
        Math.PI
...
export default {
    ...
    computed: {
        ...
        getListTop() {
            return {
                top: `${(radius - Math.round(lineHeight / 2)) / 100}rem`,
                height: `${lineHeight / 100}rem`
            };
        },
        getWrapperHeight() {
            return {
                height: `${(2 * radius) / 100}rem`
            };
        },
        getCoverStyle() {
            return {
                backgroundSize: `100% ${(radius - Math.round(lineHeight / 2)) /
                    100}rem`
            };
        },
        getDividerStyle() {
    ...
    mounted() {
        ...
        //this.updateRange(Math.round(1 / lineHeight));
        this.$refs.wheel.style.transform = `rotate3d(1, 0, 0, ${(1 /
            lineHeight) *
            singleDeg}deg)`;
    },
    beforeDestory() {
    ...
    methods: {
        initWheelItemDeg(index) {
            return {
                transform: `rotate3d(1, 0, 0, ${-1 *
                    index *
                    singleDeg}deg) translate3d(0, 0, ${radius / 100}rem)`,
                height: `${lineHeight / 100}rem`,
                lineHeight: `${lineHeight / 100}rem`
            };
        },
        ...
        listenerTouchMove(ev) {
            ...
            const move =
                ...
                this.finger.prevMove;
            this.finger.currentMove = move;
            this.$refs.wheel.style.transform = `rotate3d(1, 0, 0, ${(move /
                lineHeight) *
                singleDeg}deg)`;
        ...
        getSelectValue(move) {
            ...
            console.log("index", index, this.values.length);
        ...
        inertia(start, position, target) {
            if (start <= target || !isInertial) {
                ...
                this.finger.prevMove = this.finger.currentMove;
                this.updateRange(
                    Math.round(this.finger.currentMove / lineHeight)
                );
            ...
            this.animate.start(
                this.inertia.bind(this, newStart, position, target)
            );
        }
    }
};
